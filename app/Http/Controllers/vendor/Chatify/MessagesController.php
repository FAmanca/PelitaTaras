...
class MessagesController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        // If the user is an admin, search for all users regardless of role
        if (Auth::user()->role === 'admin') {
            $records = User::where('name', 'LIKE', "%{$input}%")
            ->paginate($request->per_page ?? $this->perPage);
        } else {
            // Otherwise, only search for users with role 'admin'
        ...
        if ($records->total() < 1) {
            $getRecords = '<p class="message-hint center-el"><span>Tidak Ada</span></p>';
        }
